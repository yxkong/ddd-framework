...
public class RequestParamsRequiredAspect {
    ...
    @Around("execution(* com.yxkong.demo..controller.*.*(..))")
        ...
        try {
            ...
            if (Objects.nonNull(requiredRealName)) {
                ...
                if (!loginToken.isRealName()) {
                    return new ResultBean.Builder<>().statusEnum(ResultStatusEnum.NO_DATA).build();
                }
            }
        } catch (Exception e) {
            logger.error("请求类:" + className + ",method=" + methodName + " ,loginToken=" + JacksonUtils.toJson(loginToken), e);
