...
public class LoginTokenUtil {
     ...
     * @param loginToken
     */
    public static void reloadLoginIngo(LoginToken loginToken) {
        SecurityContextHolder.clearContext();
    ...
    public static boolean validateRealName(LoginToken loginToken) {
        if (Objects.nonNull(loginToken) &&  Objects.nonNull(loginToken.getCustomerId()) && loginToken.getCustomerId().longValue() > 0L) {
            return true;
        }
        return false;
    }
