...
import com.yxkong.demo.infrastructure.common.util.JacksonUtils;
...
public class SplitSql {
    ...
    public SplitSqlResult getShardingSql(String sql, TableSeg tableSeg) {
        ...
        if (mappedStatement.getSqlCommandType().toString().equals(insert)) {
            ...
            if (shardVaLong == null) {
                logger.error("method[getShardingSql] INSERT分表键不能为空{}", JacksonUtils.toJson(parameterObject));
        ...
        if (mappedStatement.getSqlCommandType().toString().equals(update)) {
            ...
            Long shardVaLong = getShardingVal(tableSeg.shardBy());
            if (shardVaLong == null) {
                logger.error("method[getShardingSql] UPDATE分表键不能为空{}", JacksonUtils.toJson(parameterObject));
                throw new RuntimeException();
            }
        ...
        if (mappedStatement.getSqlCommandType().toString().equals(delete)) {
            ...
            Long shardVaLong = getShardingVal(tableSeg.shardBy());
            if (shardVaLong == null) {
                logger.error("method[getShardingSql] DELETE分表键不能为空{}", JacksonUtils.toJson(parameterObject));
                throw new RuntimeException();
            }
