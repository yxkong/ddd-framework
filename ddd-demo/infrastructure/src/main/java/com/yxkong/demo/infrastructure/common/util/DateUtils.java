...
public class DateUtils {
    ...
    public static final String DATE_SPOT_STR = "yyyy.MM.dd";
    ...
    public static String msFormatDateTime(Date dtmDate, String strFormat) {
        ...
        if (Objects.isNull(strFormat) || "".equals(strFormat)){
     ...
     * @param endDate 档期结束时间
     * @param now 要计算的时间
     * @param receiveInterval 领取时间间隔
     * @return java.util.Map<java.lang.String,java.util.Date>
     **/
    public static Map<String,Date> getTimeInterval(Date startDate, Date endDate, Date now, Integer receiveInterval){
        Map<String,Date> result=new HashMap<>(16);
        //如果没有领取间隔,或者时间超界，返回整个档期时间
        if(receiveInterval==null||endDate.before(now)||now.before(startDate)){
            result.put(START_DATE_KEY,startDate);
            result.put(END_DATE_KEY,endDate);
            return result;
        }
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(startDate);
        calendar.add(Calendar.MONTH, receiveInterval);
        Date endPeriodTime = calendar.getTime();
        //超界返回边界值
        if(endDate.before(endPeriodTime)){
            endPeriodTime=endDate;
        }
        if(startDate.before(now)&&now.before(endPeriodTime)){
            //如果符合获取当前时间所在区间
            result.put(START_DATE_KEY,startDate);
            result.put(END_DATE_KEY,endPeriodTime);
            return result;
        }else {
            startDate=endPeriodTime;
            return getTimeInterval(startDate,endDate,now,receiveInterval);
        }
    }
